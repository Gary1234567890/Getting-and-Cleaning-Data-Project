...
# put it together in the format of: subjects, labels and data
data <- rbind(cbind(test.subjects, test.labels, test.data),
              cbind(train.subjects, train.labels, train.data))

## step 2
# read the features stripping out the white spaces and making sure strings are not factored
...
## step 3
# read the acvtivity labels
labels <- read.table("activity_labels.txt", stringsAsFactors=FALSE)
# replace labels in data with label names just read in the step above
data.mean.std$label <- labels[data.mean.std$label, 2]

## step 4
# first make a list of the current column names and feature names
real.colnames <- c("subject", "label", features.mean.std$V2)
# then tidy that list
# by removing every non-alphabetic character and converting to lowercase
real.colnames <- tolower(gsub("[^[:alpha:]]", "", real.colnames))
# then use the list as column names for data
colnames(data.mean.std) <- real.colnames
